        ...
        <div class="card mb-3"><div class="card-header">Charges</div>
            <div class="table-responsive"><table class="table mb-0">
                ...
                @if($treatment->extraction_qty > 0)
                    <tr><td>Tooth Extraction ({{ $treatment->extraction_qty }} × {{ money($treatment->extraction_price) }})</td><td class="text-end">{{ money($treatment->extractionTotal()) }}</td></tr>
                @endif
                @if($treatment->implant_qty > 0)
                    <tr><td>Tooth Implant ({{ $treatment->implant_qty }} × {{ money($treatment->implant_price) }})</td><td class="text-end">{{ money($treatment->implantTotal()) }}</td></tr>
                @endif
